...
export default function PurchaseRequestForm({ onSubmit, initialData }: PurchaseRequestFormProps) {
  const { products, loading: productsLoading } = useProducts();
  const { warehouses, loading: warehousesLoading } = useWarehouses();

  const [warehouseId, setWarehouseId] = useState<number>(initialData?.warehouse_id || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setError(err.response?.data?.error || err.message || 'Failed to save purchase request');
