...
const purchaseRequestController = {
  ...
  async getPurchaseRequestById(req, res, next) {
    try {
      ...
      if (!result) {
        return res.status(404).json({ error: `Purchase request id ${id} not found` });
      }
  ...
  async createPurchaseRequest(req, res, next) {
    try {
      const result = await purchaseRequestService.createPurchaseRequest(req.body);
  ...
  async updatePurchaseRequest(req, res, next) {
    try {
      const { id } = req.params;
      const result = await purchaseRequestService.updatePurchaseRequest(id, req.body);
  ...
  async deletePurchaseRequest(req, res, next) {
    try {
      ...
      res.status(200).json({ message: `Purchase request id ${id} deleted successfully` });
    } catch (error) {
      next(error);
    }
  },
