  ...
  async handleDone(reference, data) {
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      if (purchaseRequest.status === 'COMPLETED') {
        console.warn(`Purchase request ${reference} is already completed (idempotency check)`);
      ...
      for (const item of details) {
        ...
        if (!product) {
          console.warn(`Product with SKU ${sku_barcode} not found, skipping...`);
          continue;
        }
        ...
        console.log(`Updated stock for ${product.name} (${sku_barcode}): +${qty}`);
      }
