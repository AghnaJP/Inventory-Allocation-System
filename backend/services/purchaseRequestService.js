...
const purchaseRequestService = {
  ...
  async updatePurchaseRequest(id, data) {
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      console.log('After update, purchaseRequest.status is now:', purchaseRequest.status);
  ...
  async triggerFoomWebhook(purchaseRequest) {
    ...
    if (!FOOM_URL) throw new Error('FOOM_URL is not defined in environment variables');
    if (!FOOM_SECRET_KEY)
      throw new Error('FOOM_SECRET_KEY is not defined in environment variables');
    ...
    try {
      ...
      console.error('FOOM webhook error:', error.response?.data || error.message);
      throw error;
    }
  },
